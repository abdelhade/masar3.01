...
class ReportController extends Controller
{
    ...
    public function inventoryDiscrepancyReport()
    {
        ...
        $warehouses = AccHead::where('isdeleted', 0)
            ->where('is_basic', 0)
            ->where('code', 'like', '123%')
            ->select('id', 'aname')
            ->get();;
        ...
        foreach ($items as $item) {
            // حساب الكمية الحالية في النظام
            $systemQuantity = $this->calculateItemBalance($item->id, $selectedWarehouse);

            // الكمية المتوقعة (يمكن أن تكون من جدول منفصل أو من حقل في جدول الأصناف)
            $expectedQuantity = $item->expected_quantity ?? $systemQuantity;

            // الكمية الفعلية (يتم إدخالها من المستخدم)
            $actualQuantity = request('actual_quantity_' . $item->id, $systemQuantity);
            ...
            $discrepancyType = '';
            if ($discrepancy > 0) {
                $discrepancyType = 'زيادة';
            } elseif ($discrepancy < 0) {
                $discrepancyType = 'نقص';
            } else {
                $discrepancyType = 'متطابق';
            }
            ...
            $inventoryData->push([
                ...
                'discrepancy_value' => abs($discrepancy) * ($item->cost ?? 0)
            ]);
        }

        // إحصائيات التقرير
        $totalItems = $inventoryData->count();
        $itemsWithShortage = $inventoryData->where('discrepancy', '<', 0)->count();
        $itemsWithOverage = $inventoryData->where('discrepancy', '>', 0)->count();
        $itemsMatching = $inventoryData->where('discrepancy', '=', 0)->count();
        $totalShortageValue = $inventoryData->where('discrepancy', '<', 0)->sum('discrepancy_value');
        $totalOverageValue = $inventoryData->where('discrepancy', '>', 0)->sum('discrepancy_value');

        return view('reports.items.inventory-discrepancy-report', compact(
            'inventoryData',
            'warehouses',
            'selectedWarehouse',
            'totalItems',
    ...
    public function update(Request $request)
    {
        ...
        $selectedWarehouse = $request->input('warehouse_id', null);

        // حساب الكمية الحالية في النظام
        $systemQuantity = $this->calculateItemBalance($itemId, $selectedWarehouse);
        $expectedQuantity = $item->expected_quantity ?? $systemQuantity;
        $discrepancy = $actualQuantity - $expectedQuantity;
        ...
        // هنا يمكنك إضافة الكود لحفظ الكمية الفعلية في قاعدة البيانات إذا لزم الأمر
        // على سبيل المثال: $item->update(['actual_quantity' => $actualQuantity]);
    ...
    public function updateAll(Request $request)
    {
        ...
        foreach ($quantities as $itemId => $actualQuantity) {
