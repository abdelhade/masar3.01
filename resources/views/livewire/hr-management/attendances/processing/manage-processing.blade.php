            ...
            <div class="card">
                <div class="card-header">
                    <div class="row align-items-end g-2">
                        <div class="col-md-2">
                            <h5 class="font-family-cairo fw-bold mb-0">{{ __('سجلات معالجة الحضور') }}</h5>
                        </div>

                        <div class="col-md-2">
                            <x-tom-select name="filter_employee" :options="$employees->map(fn($p) => ['value' => $p->id, 'text' => $p->name])->toArray()"
                                wire:model.live.debounce.500ms="search_employee_name"
                                placeholder="{{ __('اختر الموظف') }}" :search="true" dir="rtl"
                                class="font-family-cairo form-control " :multiple="false" :required="true"
                        ...
                        <div class="col-md-2">
                            <x-tom-select name="filter_department" :options="$departments->map(fn($p) => ['value' => $p->id, 'text' => $p->title])->toArray()"
                                wire:model.live.debounce.500ms="search_department_name"
                                placeholder="{{ __('اختر القسم') }}" :search="true" dir="rtl"
                                class="font-family-cairo form-control " :multiple="false" :required="true"
                                :syncOnModalOpen="true" :clearOnModalClose="true" :tomOptions="[
                                    'plugins' => [
                                        'dropdown_input' => ['class' => 'font-family-cairo fw-bold font-14'],
