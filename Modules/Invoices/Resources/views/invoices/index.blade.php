                    ...
                    <div class="table-responsive" style="overflow-x: auto;">
                        <table id="invoices-table" class="table table-striped mb-0" style="min-width: 1200px;">
                            <thead class="table-light text-center align-middle">
                                ...
                                <tr>
                                    <th><input type="text" x-model="filters.index" class="form-control form-control-sm" placeholder="#"></th>
                                    <th><input type="text" x-model="filters.date" class="form-control form-control-sm" placeholder="{{ __('Date') }}"></th>
                                    @if (!in_array($invoiceType, [18, 19, 20, 21]))
                                        <th><input type="text" x-model="filters.dueDate" class="form-control form-control-sm" placeholder="{{ __('Due Date') }}"></th>
                                    @endif
                                    <th><input type="text" x-model="filters.operationName" class="form-control form-control-sm" placeholder="{{ __('Operation Name') }}"></th>
                                    <th><input type="text" x-model="filters.account" class="form-control form-control-sm" placeholder="{{ __('Account') }}"></th>
                                    <th><input type="text" x-model="filters.counterAccount" class="form-control form-control-sm" placeholder="{{ $invoiceType == 21 ? __('Counter Store') : __('Counter Account') }}"></th>
                                    <th><input type="text" x-model="filters.employee" class="form-control form-control-sm" placeholder="{{ __('Employee') }}"></th>
                                    <th><input type="text" x-model="filters.financialValue" class="form-control form-control-sm" placeholder="{{ __('Financial Value') }}"></th>
                                    @if (!in_array($invoiceType, [18, 19, 20, 21]))
                                        <th><input type="text" x-model="filters.paidAmount" class="form-control form-control-sm" placeholder="{{ in_array($invoiceType, [11, 13, 15, 17]) ? __('Paid to Supplier') : __('Paid by Customer') }}"></th>
                                    @endif
                                    <th><input type="text" x-model="filters.netOperation" class="form-control form-control-sm" placeholder="{{ __('Net Operation') }}"></th>
                                    @if (!in_array($invoiceType, [11, 13, 18, 19, 20, 21]))
                                        <th><input type="text" x-model="filters.profit" class="form-control form-control-sm" placeholder="{{ in_array($invoiceType, [11, 13, 15, 17]) ? __('Cost') : __('Profit') }}"></th>
                                    @endif
                                    <th>
                            ...
                            <tbody>
                                @php
                                    $invoicesData = $invoices->map(function($invoice, $index) use ($invoiceType) {
                                        $totalAmount = $invoice->pro_value;
                                        $paidAmount = $invoice->paid_from_client;
                                        
                                        if ($paidAmount == 0) {
                                            $paymentStatus = 'unpaid';
                                            $paymentBadge = '<span class="badge bg-danger">' . __('Unpaid') . '</span>';
                                        } elseif ($paidAmount >= $totalAmount) {
                                            $paymentStatus = 'paid';
                                            $paymentBadge = '<span class="badge bg-success">' . __('Paid') . '</span>';
                                        } else {
                                            $paymentStatus = 'partial';
                                            $paymentBadge = '<span class="badge bg-warning text-dark">' . __('Partial') . '</span>';
                                        }
                                        ...
                                        return [
                                            ...
                                            'dueDate' => \Carbon\Carbon::parse($invoice->accural_date)->format('Y-m-d'),
                                ...
                                <template x-for="(invoice, idx) in filteredInvoices" :key="invoice.id">
                                    <tr>
                                        ...
                                        <td><span class="badge bg-light text-dark" x-text="invoice.counterAccount"></span></td>
                ...
                get filteredInvoices() {
                    return this.invoices.filter(invoice => {
                        ...
                        const matchOperationName = !this.filters.operationName || 
                            invoice.operationName.toLowerCase().includes(this.filters.operationName.toLowerCase());
                        ...
                        const matchCounterAccount = !this.filters.counterAccount || 
                            invoice.counterAccount.toLowerCase().includes(this.filters.counterAccount.toLowerCase());
