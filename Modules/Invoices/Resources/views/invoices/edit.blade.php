...
@section('content')
    ...
    {{-- Pure HTML - No Alpine --}}
    <div id="invoice-app">
        <form id="invoice-form" method="POST" action="@if(isset($isEditMode) && $isEditMode){{ route('invoices.update', $invoiceId) }}@else{{ route('invoices.store') }}@endif">
            @csrf
    ...
    <script>
        ...
        window.InvoiceApp = {
            ...
            init() {
                ...
                @if(isset($isEditMode) && $isEditMode)
                this.loadInvoiceData(); // Load existing invoice data first
                @endif
                this.loadItems();
            ...
            setDefaultValues() {
                ...
                // Set default store
                const defaultStoreId = '{{ $defaultAcc2Id ?? '' }}';
                if (defaultStoreId) {
                    const acc2Element = document.getElementById('acc2-id');
                    if (acc2Element) {
                        acc2Element.value = defaultStoreId;
                    }
                }
                ...
                if (invoiceData.operation_items && invoiceData.operation_items.length > 0) {
                    invoiceData.operation_items.forEach((item, index) => {
                        ...
                        const invoiceItem = {
                            ...
                            discount_value: parseFloat(item.item_discount || 0),
                            sub_value: parseFloat(item.detail_value || 0),
