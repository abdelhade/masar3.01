        ...
        <div class="col-12">
            <div class="page-title-box no-print">
                <div class="d-flex justify-content-between align-items-center">
                    <h4 class="page-title">{{ __('Work Item Details') }}: {{ $workItem->name ?? '#' . $workItem->id }}</h4>
                    <div class="d-flex gap-2">
                        <a href="{{ route('work.items.edit', $workItem) }}" class="btn btn-primary">
                            <i class="fas fa-edit"></i> {{ __('Edit') }}
                        </a>
                        <button onclick="window.print()" class="btn btn-info">
                            <i class="fas fa-print"></i> {{ __('Print') }}
                        </button>
                        <a href="{{ route('work.items.index') }}" class="btn btn-secondary">
        ...
        <div class="col-12">
            <div class="card printable-content">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0"><i class="fas fa-tasks"></i> {{ __('Work Item Information') }}</h5>
                </div>
                <div class="card-body">
                    <div class="row">
                        @foreach($workItem->getAttributes() as $key => $value)
