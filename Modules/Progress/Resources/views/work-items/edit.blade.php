            ...
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-primary text-white d-flex align-items-center justify-content-between py-3">
                    <h5 class="mb-0 text-white fw-bold">
                        ...
                        {{ __('general.edit_work_item') }}: {{ $workItem->name }}
                    </h5>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('work.items.update', $workItem) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-lg-12">
                                ...
                                <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="4">{{ old('description', $workItem->description) }}</textarea>
                                @error('description')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
